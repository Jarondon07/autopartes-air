...
- **Worker de tasas:** job in-process en el server (no es un app aparte) que consulta la API de Radar **una vez al día a las `BCV_FETCH_TIME`** (default `00:30`, hora local) y actualiza las 3 tasas; además hay una actualización manual ("Actualizar ahora" → `POST /exchange-rates/refresh`). Ver [apps/server/src/modules/exchange-rates/bcv-worker.ts](apps/server/src/modules/exchange-rates/bcv-worker.ts).
...
- **Tasas de cambio:** 3 fuentes (`bcv`, `euro`, `usdt`), a **2 decimales**. Hubo una cuarta, `intervencion`, **eliminada en sep. 2026** (migración 0026, borra el histórico y saca el valor del `pgEnum`): Radar dejó de publicarla y la última quedó congelada el 13/08/2026 — una tasa que no se actualiza miente más de lo que informa. Para quitar un valor de un `pgEnum` hay que recrear el tipo, y el `ALTER ... USING` **falla si queda alguna fila con ese valor**: el `DELETE` va primero en la migración. El worker las trae de Radar **una vez al día a `BCV_FETCH_TIME`** (default `00:30`) + una consulta al arrancar; automáticas = `createdBy null`, manuales = id del usuario. Botón **"Actualizar ahora"** en `Configuración → Tasas` (`POST /exchange-rates/refresh`, permiso `RATES_CREATE`) para forzar la consulta. Fallback `BCV_FALLBACK_RATE` si no hay ninguna.
...
- **Inversión y ganancia:** "invertido" son dos cosas distintas y se muestran las dos — lo **comprado** en el período (`purchases/summary`, dinero que salió, desglosado por proveedor) y el **capital parado** en inventario (`inventory/summary`). La **ganancia bruta** es `SUM(sale_details.subtotalUsd) - SUM(cantidad × costo)`, **sin IVA** (el IVA no es ingreso) y solo sobre ventas `completada`. Falta el módulo de **gastos**, así que es utilidad **bruta**, nunca neta.
- **`sale_details.unitCostUsd` (costo congelado):** foto del costo al vender, igual que `exchangeRate` lo es de la tasa. `products.costUsd` es el costo del **último** lote y lo **pisa cada compra**: sin congelarlo, la ganancia de una venta vieja se recalcularía con un costo que entró después (vendiste a 60 con costo 40, compraste a 50 → el reporte diría 10 en vez de 20). Es **nullable**: `null` = venta anterior a la columna, y los reportes caen a `products.costUsd` con `coalesce`. Por eso `salesProfit` devuelve `estimatedLines`, y la pantalla avisa con un `Alert` cuando parte del rango es estimado. El costeo real por lote (FIFO / promedio ponderado) sobre `purchase_details` queda pendiente.
...
- **Fase 2** ✅ CRUD backend + web completo: Productos, Categorías (con activar/desactivar), Clientes, Proveedores, Tasas (3 fuentes + worker BCV). Shell del layout (sidebar con menú anidado, topbar con perfil, footer). Perfil propio (`/perfil`, editar nombre + cambiar contraseña).
...
- **Fase 4** ⏳ Dashboard real ✅, Reportes ✅ (ventas por día, top productos, cuadre por método, inventario valorizado, **inversión y ganancia bruta** — sin gráficos aún) y **Deudas** ✅ (ventas a crédito con fecha de pago, abono inicial, abonos y estados derivados). Pendiente: **gráficos**, **impresión de recibos**, **módulo de gastos** (para llegar a la ganancia neta) y **costeo por lote**.
...
- La API de Radar devuelve `{ rates: [...] }` (no un array plano) y `midRate` como **string** — el worker lo parsea así. El endpoint es **`https://radar.evolut.team/api/rates`** (el viejo `radar.revolut.team` solo redirige).
- **Una redirección a otro dominio te borra el `Authorization`.** `fetch` (undici) sigue los 3xx pero **elimina `Authorization`, `Cookie` y `Proxy-Authorization` al cambiar de origen**, para no filtrar credenciales a terceros; los headers *personalizados* sí sobreviven. Radar movió su dominio en sep. 2026 y el worker pasó dos días recibiendo **401 con una clave válida**: el request llegaba al destino sin credencial. Por eso la clave viaja en `x-api-key`. Ante un 401 contra un servicio externo, comprobar primero si hay un 3xx de por medio: `curl -sS -o /dev/null -D - <url> | grep -i '^location'`.
- `.env` y `.env.local` están en `.gitignore` (contienen secretos, incl. `RADAR_API_KEY`). Al clonar, copiar de `.env.example`. **Credenciales y endpoints de terceros viven en el `.env`, nunca como default en `infra/env.ts`**: un default en el código tapa un `.env` desactualizado y obliga a tocar código (y redesplegar) cuando el proveedor cambia de dominio. Por eso `RADAR_API_URL` no tiene default y se exige **junto con** `RADAR_API_KEY` (`superRefine`): sin clave el server arranca igual (worker apagado); con clave y sin URL falla **al arrancar**, con mensaje, y no en la consulta de las 00:30.
...
- **Un módulo con permisos nuevos necesita `db:seed:prod`, no solo la migración.** Los permisos viven en filas (`permissions` + `role_permissions`): mientras no se siembren, el módulo existe pero **nadie lo ve**, ni siquiera `admin`. Pasó con `debts:read`/`debts:pay`. El seed es idempotente (solo agrega lo que falta, no pisa usuarios ni contraseñas), así que correrlo en cada despliegue que toque `shared/constants/roles.ts` es seguro:
  ...
  ```
- **`.env` no viaja con git: las variables nuevas o cambiadas se aplican a mano en el VPS.** Está en `.gitignore`, así que un `git pull` nunca lo toca. Si el código empieza a exigir una variable (o cambia el valor esperado) y nadie edita el `.env` de producción, la API **no arranca** tras el restart. Pasó al mover `RADAR_API_URL` fuera del código. Revisar el `.env.example` del commit antes de reiniciar.
